...
class TestScipyOptimizeDriver(unittest.TestCase):
    ...
    def test_normalize_design_vars_COBYLA_multiscale(self):
        ...
        # Sanity check: without normalization and the same small rhobeg, COBYLA doesn't
        # move the large-magnitude DV, so the optimum is not reached in a comparable budget.
        prob2 = om.Problem()
        ...
        # Without normalization, rhobeg=0.1 is far too small to move x_big meaningfully,
        # so x_big should remain nowhere near the true optimum of 1500.
        self.assertGreater(abs(prob2.get_val('x_big').item() - 1500.0), 100.0,
                           "Unnormalized COBYLA with tiny rhobeg unexpectedly reached the "
                           "optimum; the multiscale test is not exercising the intended "
                           "code path.")
    ...
    def test_normalize_design_vars_requires_finite_bounds(self):
        prob = om.Problem()
        model = prob.model
        ...
        self.assertIn("requires finite lower and upper bounds", str(ctx.exception))
