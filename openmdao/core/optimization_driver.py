...
from openmdao.core.driver_result import OptimizationResult
...
class OptimizationDriver(Driver):
    ...
    **kwargs : dict of keyword arguments
    ...
    result : OptimizationResult
        Object containing information and statistics on the optimimization.
    _has_scaling : bool
    ...
    def _compute_totals(self, of=None, wrt=None, return_format='flat_dict', driver_scaling=True):
        ...
        if debug_print:
            header = f'Driver total derivatives for iteration: {self.iter_count}'
            print(header)
            print(len(header) * '-' + '\n')
...
class SaveOptResult(object):
    ...
    def __exit__(self, *args):
        ...
        *args : array
        ...
        """
        driver = self._driver
        opt_result = driver.result
        opt_result.runtime = time.perf_counter() - self._start_time
        opt_result.iter_count = driver.iter_count
        opt_result.obj_calls = driver.get_driver_objective_calls()
        opt_result.deriv_calls = driver.get_driver_derivative_calls()
        opt_result.exit_status = driver.get_exit_status()
